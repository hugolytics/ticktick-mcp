...
class TickTickClientSingleton:
    ...
    def __new__(cls):
        ...
        return super(TickTickClientSingleton, cls).__new__(
            cls
        )  # Return the instance (or create if needed)
    ...
    def __init__(self):
        ...
        if not all([CLIENT_ID, CLIENT_SECRET, REDIRECT_URI, USERNAME, PASSWORD]):
            logging.error(
                "TickTick credentials not found in environment variables (checked in config.py). Ensure .env file is correct."
            )
            TickTickClientSingleton._instance = (
                None  # Ensure instance is None if creds are missing
            )
            TickTickClientSingleton._initialized = (
                True  # Mark as initialized (attempted)
            )
            return
        ...
        try:
            ...
            logging.error(
                f"Error initializing TickTick client within singleton: {e}",
                exc_info=True,
            )
    ...
    def get_client(cls) -> Optional[TickTickClient]:
        ...
        if cls._instance is None:
            logging.warning(
                "get_client() called, but TickTick client failed to initialize."
            )
